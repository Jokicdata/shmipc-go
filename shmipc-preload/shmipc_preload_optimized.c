...
#define MAX_FDS 4096
#define MAX_PATH 256
#define SHM_BUFFER_SIZE (256 * 1024 * 1024)
#define QUEUE_CAPACITY 8192
#define BATCH_IOV_MAX 16
...
extern long ShmipcWriteBatch(int stream_id, const struct iovec *iov, int iovcnt);
extern long ShmipcReadBatch(int stream_id, const struct iovec *iov, int iovcnt);
extern int ShmipcCloseStream(int stream_id);
extern int ShmipcCloseSession(int fd);
extern int ShmipcGetStats(void *stats);
extern void ShmipcPrintStats(void);

/* ========== 全局状态 ========== */
static int g_log_level = LOG_ERROR;
static int g_initialized = 0;
static int g_shmipc_enabled = 1;
static int g_batch_io_enabled = 1;
...
static struct {
    ...
    uint64_t batch_write_ops;
    uint64_t batch_read_ops;
} g_stats;
...
static void lib_init(void) {
    ...
    char *batch_env = getenv("SHMIPC_BATCH_IO");
    if (batch_env && strcmp(batch_env, "0") == 0) {
        g_batch_io_enabled = 0;
    }
    ...
    g_initialized = 1;
    log_msg(LOG_INFO, "shmipc-preload loaded (enabled: %d, batch_io: %d, log: %d)", 
            g_shmipc_enabled, g_batch_io_enabled, g_log_level);
}
