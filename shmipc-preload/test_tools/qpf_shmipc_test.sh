...
LOG_FILE="$LOG_DATE"_qpf_shmipc_bench_test.log
...
for MSG_SIZE in "${MSG_SIZES[@]}"; do
    ...
    date | tee -a $LOG_FILE
    LD_PRELOAD=../libshmipc.so qperf 127.0.0.1 -m $MSG_SIZE -t 120 | tee -a $LOG_FILE
    date | tee -a $LOG_FILE
    echo "" | tee -a $LOG_FILE
...
echo "Stopping qperf server..." | tee -a $LOG_FILE
kill $QPERF_PID
